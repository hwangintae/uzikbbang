...
public class CartItemServiceTest {
    ...
    @Test
    @DisplayName("장바구니에서 주문을 했을 경우 useAt를 false로 업데이트 해야한다.")
    void testUseAtFalse() {
        SignUp signUp = SignUp.builder()
            .name("황인태")
            .email("hwang@hwang.com")
        ...
        orderService.addOrderProducts(user.getId());

        CartItem resultCartItem1 = cartItemService.findById(cartItem1.getId()).get();
        CartItem resultCartItem2 = cartItemService.findById(cartItem2.getId()).get();
        CartItem resultCartItem3 = cartItemService.findById(cartItem3.getId()).get();

        assertEquals(false, resultCartItem1.getUseAt());
        assertEquals(false, resultCartItem2.getUseAt());
        assertEquals(false, resultCartItem3.getUseAt());
    }
